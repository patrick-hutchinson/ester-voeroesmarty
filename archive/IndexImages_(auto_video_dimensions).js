...
export default function IndexImages({ projects, page }) {
    ...
    useEffect(() => {
        ...
        // Collect all promises to fetch video dimensions
        const fetchPromises = projects.map((practice, i) => {
            const isVideo = practice.attributes.cover.data.attributes.url.includes('.mp4', 'mov', 'avi', 'flv', 'wmv', '3gp', 'mkv', 'webm');
            if (isVideo) {
                ...
                return getVideoDimensions(process.env.NEXT_PUBLIC_STRAPI_API_URL + practice.attributes.cover.data.attributes.url)
                    .then(({ width, height }) => ({ practice, i, isVideo, width, height }))
                    .catch((error) => {
                ...
                return Promise.resolve({ practice, i, isVideo, width: origImgW, height: origImgH });
            }
        });
        ...
        Promise.all(fetchPromises).then((results) => {
            results.forEach((result) => {
                ...
                const { practice, i, isVideo, width: origImgW, height: origImgH } = result;
