...
export default function IndexTitleAndCounter({ projects }) {
    ...
    useEffect(() => {
        const checkTitleTop = () => {
            const threshold = 64;
            document.querySelector('.proxy') && Array.from(document.querySelector('.proxy').children).forEach((item, i) => {
    ...
    return (
        <>
            <div className="practice-title">
                <h1>
                    {titleCounter < projectsWithCover.length ? projectsWithCover[titleCounter].theme : 'About'}
                </h1>
            </div>
